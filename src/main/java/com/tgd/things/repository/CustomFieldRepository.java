...
public interface CustomFieldRepository extends CrudRepository<CustomField, String> {
	...
	/*
	 * SELECT tty_id, tty_name, cfi_key, cfi_name, cfi_type FROM CUSTOM_FIELD
	 * cfi left JOIN THING_TYPE_CUSTOMFIELD ttc ON cfi_id=ttc.customfield_cfi_id
	 * left JOIN THING_TYPE tty ON tty_id=ttc.thingtype_tty_id where
	 * tty.TTY_ID=1
	 */
	@Query(value = "SELECT cfi_id as id, cfi_key as key, cfi_name as name, cfi_type as type FROM custom_field cfi "
	...
	@Query(value = "SELECT DISTINCT cfi_id as id, cfi_key as key, cfi_name as name, cfi_type as type, cfv_value as value FROM thing THI "
